...
| `PROBE_MATCH_T` | 0.28 | seuil (plus tolérant) pour les sondes temps réel — lecture seule, ne pollue jamais les centroïdes |
...
**c) Auto-enrôlement** (`auto_enroll_voices`, appelé à `disconnect`) — si un locuteur a été confirmé par vote LLM avec ≥ 8 segments (`VOICE_ENROLL_MIN_SEGMENTS`) et fait partie des invités déclarés, son centroïde est sauvegardé en banque. **La banque s'auto-enrichit à chaque débat** : au suivant, la même personne est reconnue acoustiquement dès le premier segment, sans passer par le vote LLM.
