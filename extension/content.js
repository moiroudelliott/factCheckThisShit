...
//
// Le même fichier tourne sur la page « relecture » du site (site/overlay/,
// copie synchronisée par publish_session.py) : la page y rejoue une session
// enregistrée (voir « Enregistrement pour la relecture »).
// ══════════════════════════════════════════════════════════════════════════════
...
chrome.runtime.onMessage.addListener((msg, _sender, sendResponse) => {
  ...
  try {
    ...
    if (msg.type === 'talking_points')    (msg.points || []).forEach(p => addPoint(p, { card: !msg.backlog }));
...
function addPoint(point, { card = true } = {}) {
  ...
  // (déjà connue quand le point vient d'une session enregistrée)
  const saidAt = Number.isFinite(point.said_at) ? point.said_at
    : Number.isFinite(point.ts) ? point.ts - SPEECH_LEAD_S : null;
  if (!Number.isFinite(point.vt)) point.vt = saidAt != null ? videoTimeAt(saidAt) : null;
...
function downloadFile(content, type, prefix, ext) {
  const blob = new Blob([content], { type });
  const a = document.createElement('a');
  a.href = URL.createObjectURL(blob);
  const suffix = IS_YOUTUBE ? S.videoId || '' : location.hostname.replace(/^www\./, '');
  a.download = `${prefix}_${new Date().toISOString().slice(0, 10)}${suffix ? '_' + suffix : ''}.${ext}`;
  a.click();
  setTimeout(() => URL.revokeObjectURL(a.href), 5000);
}

// ── Enregistrement pour la relecture ──────────────────────────────────────────
// Chaque message du backend est gardé avec la position vidéo à laquelle il
// est arrivé. Rejoué aux mêmes positions sur la page « relecture » du site
// (lecteur YouTube intégré, sans GPU ni backend), il reproduit l'analyse
...
function record(msg) {
  ...
  if (msg.type === 'talking_points') {
    // Position du propos calculée à la réception : la relecture la reprend
    // telle quelle ; les horodatages unix du direct ne servent plus à rien
    msg.points = (msg.points || []).map(({ ts, said_at, ...p }) => ({ ...p, vt: S.points.get(p.id)?.point.vt ?? null }));
  } else if (msg.type === 'speaker_live' || msg.type === 'transcript_segment') {
    msg = { type: msg.type, speaker: msg.speaker }; // le badge n'a besoin que du locuteur
  }
  S.tape.push({ t: Math.round(t * 10) / 10, m: msg });
}
...
function openRecap() {
  ...
  panel.querySelector('#fct-recap-session').addEventListener('click', exportSession);
